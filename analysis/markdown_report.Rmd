---
title: "Lipids Audit"
author: "Rob Willans"
date: "15/03/2022"
output: 
  html_document:
    toc: true
    toc_float: true
---

```{r setup, include=FALSE}
library(tidyverse)
library(janitor)

knitr::opts_chunk$set(echo = TRUE)


## Load data from cohort extractor
df_input <- read_csv(here::here("output", "input.csv")) %>%
  mutate(
    "age_group" = case_when(
      age < 40 ~ "18-39",
      age < 60 ~ "40-59",
      age < 80 ~ "60-79",
      age >= 80 ~ "80-84",
      TRUE ~ "No age"
    ),
    "CVD_assess_cat" = case_when(
      CVD_assess_latest_number > 0 ~ "CVD risk assessment",
      (is.na(CVD_assess_latest_number)|CVD_assess_latest_number == 0) ~ "No CVD risk assessment number",
      TRUE ~ "Negative or Invalid Input"
    ),
    "cvd_statin_cat" = case_when(
      is.na(cvdprevent_statins_issued) ~ "No statin recorded (CVDPREVENT codelist)",
      TRUE ~ "Statins recorded (CVDPREVENT codelist)"
    ),
    "cvd_statin_cat_last_6m" = case_when(
      is.na(cvdprevent_statins_issued_last_6m) ~ "No statin recorded in last 6m (CVDPREVENT codelist)",
      TRUE ~ "Statins recorded in last 6m (CVDPREVENT codelist)"
    ),
  )
  

summarise_df_by_group <- function(df, ...){
  df %>% 
    group_by(...) %>% 
    summarise(n=n()) %>% 
    ungroup() %>% 
    mutate("percentage" = sprintf("%.2f%%", n/sum(n)*100)) %>%
    adorn_totals()
  }

```

# Initial counts

## Total N (aged 18-84 inclusive at end of 2021)

```{r}

nrow(df_input)

```

## N with CVD diagnosis 

* CVD codelist taken from CVDPrevent definitions, including stroke codes,  TIA codes, CHD codes, PAD codes and excluding HF and AAA codes

```{r CVD_n}

df_input %>%
  filter(CVD_code_CVD_prevent == 1) %>% 
  summarise_df_by_group(age_group)

```

## N with CKD diagnosis 

```{r CKD_n}

df_input %>%
  filter(CKD_code == 1) %>% 
  summarise_df_by_group(age_group)

```

## N with T1D diagnosis

```{r T1D_n}

df_input %>%
  filter(T1D_code == 1) %>% 
  summarise_df_by_group(age_group)

```

## N with T2D diagnosis

```{r T2D_n}

df_input %>%
  filter(T2D_code == 1) %>% 
  summarise_df_by_group(age_group)

```

## N with any DM diagnosis

```{r anyDcode_n}

df_input %>%
  filter(Overall_diab_code == 1) %>% 
  summarise_df_by_group(age_group)

```

## N total statin issues (CVDPREVENT statin list)

```{r totalstatinscvdp}

df_input %>% 
  summarise_df_by_group(cvd_statin_cat)

```
## N total statin issues in last 6m (CVDPREVENT statin list)

```{r totalstatinscvdp6m}

df_input %>% 
  summarise_df_by_group(cvd_statin_cat_last_6m)

```

## N excluding CVD, CKD, T1D (aged 40-84 at 2022-01-01)

```{r noCVDCKDT1D}

df_input %>% 
  filter(age >= 40,
         CVD_code_CVD_prevent == 0,
         CKD_code == 0, 
         T1D_code == 0) %>% 
  nrow()

```
## N excluding CVD, CKD, T1D and risk assessment more than 5 years ago [Indicator 1]

•	How many people aged 40-84y (excluding pre-existing CVD, T1DM and CKD) have had a CVD risk assessment recorded in the preceding 5 years?

```{r excluderiskassessmorethanfiveyears}

df_input %>% 
  filter(age >= 40,
         CVD_code_CVD_prevent == 0,
         CKD_code == 0, 
         T1D_code == 0,
         CVD_assess_latest_date >= "2016-12-31",
         CVD_assess_latest_date <= "2021-12-31",) %>% 
  nrow()

```
## N with no cvd_assess_number

```{r nocvdassess}

nrow(df_input %>% filter(CVD_assess_latest_number == 0))

```
# Calculations

## Calculate % patients with CVD assessment at any time prior to 31st Dec 2021

```{r addCVDriskcat}

df_input %>% 
  summarise_df_by_group(CVD_assess_cat)
         
```

## Calculate % patients (by age group) with CVD risk over 10 with statin issue in last 6m

3.	How many people with a recorded CVD risk assessment score of ≥ 10% in last 5 years are currently treated (defined as issue in last 6m) with a lipid modifying therapy. 

```{r CVDgreaterthan10statins}

df_input %>% 
  filter(
    CVD_assess_latest_number >= 10,
    CVD_code_CVD_prevent == 0,
    CKD_code == 0, 
         ) %>% 
  summarise_df_by_group(cvd_statin_cat_last_6m) 

```

```{r CVDgreaterthan10statinsage}

df_input %>% 
  filter(CVD_assess_latest_number >= 10,
         CVD_code_CVD_prevent == 0,
         CKD_code == 0, 
         ) %>% 
  summarise_df_by_group(cvd_statin_cat_last_6m, age_group) 

```

## Calculate % patients with CVD risk over 20 with statin issue in last 6m

4.	How many people with a recorded CVD risk assessment score of ≥ 20% in last 5 years are currently treated (defined as issue within last 6m) with a lipid modifying therapy. 

```{r CVDgreaterthan20statinsalt}

df_input %>% 
  filter(CVD_assess_latest_number >= 20,
         CVD_code_CVD_prevent == 0,
         CKD_code == 0) %>% 
  summarise_df_by_group(cvd_statin_cat_last_6m) 

```

```{r CVDgreaterthan20statinsaltage}

df_input %>% 
  filter(CVD_assess_latest_number >= 20,
         CVD_code_CVD_prevent == 0,
         CKD_code == 0) %>% 
  summarise_df_by_group(cvd_statin_cat_last_6m, age_group) 

```

## Calculate % patients with prexisting CVD/CKD currently treated with lipid modifying therapy.

5.	How many people with pre-existing CVD or CKD are currently treated (defined as issue within last 6 months) with a lipid modifying therapy.

```{r CVDCKDstatins6m}

df_input %>% 
  filter(
         CVD_code_CVD_prevent == 1 | CKD_code == 1
         ) %>% 
  summarise_df_by_group(cvd_statin_cat_last_6m) 

```
